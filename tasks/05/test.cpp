...
class CVATRegister
{
  ...
  private:
    ...
    mutable set<CInvoice, CmpInv> issued;
    mutable set<CInvoice, CmpInv> accepted;
...
bool CVATRegister::registerCompany(const std::string &name) {
   ...
   /* auto item = names.insert(normalisedName);
    return item.second;*/
...
bool CVATRegister::addIssued(const CInvoice & x) {
    ...
    auto issuedItem = issued.find(ins);
    if (issuedItem == issued.end()){
        ins.setAddedState(counter);
        counter += 1;
        auto inName = issued.insert(ins);
...
bool CVATRegister::addAccepted(const CInvoice& x) {
    ...
    CInvoice ins = CInvoice(x.date(), seller->second, buyer->second, x.amount(), x.vat());
    auto issuedItem = accepted.find(ins);
    if (issuedItem == accepted.end()){
        ins.setAddedState(counter);
        counter += 1;
        auto inName = accepted.insert(ins);
...
bool CVATRegister::delIssued(const CInvoice &x) {
    ...
    auto issuedItem = issued.find(ins);
    if (issuedItem != issued.end()){
        auto it = issued.erase(issuedItem);
        return true;
    }
...
bool CVATRegister::delAccepted(const CInvoice &x) {
    ...
    auto issuedItem = accepted.find(ins);
    if (issuedItem != accepted.end()){
        auto it = accepted.erase(issuedItem);
        return true;
    }
...
list<CInvoice> CVATRegister::unmatched(const string &company, const CSortOpt &sortBy) const {
    ...
    set_difference(issued.begin(), issued.end(),
                   accepted.begin(), accepted.end(),
                   inserter(diff, diff.end()));

    set_difference(accepted.begin(), accepted.end(),
                   issued.begin(), issued.end(),
                   inserter(diff, diff.end()));

    for (const auto & it : diff){
        if (it.buyer() == buyer->second || it.seller() == buyer->second){
